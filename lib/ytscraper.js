...
async function savetube(link, quality, value) {
    try {
        ...
        if (!cdn) {
            return { status: false, message: 'CDN no disponible' }
        }
        ...
        if (!infoget?.data) {
            return { status: false, message: 'No se obtuvo metadata cifrada' }
        }

        const info = decode(infoget.data)
        if (!info?.key) {
            return { status: false, message: 'No se obtuvo clave de descarga' }
        }
        ...
        if (!downloadUrl) {
            console.error("SaveTube error: No downloadUrl in response", JSON.stringify(response?.data))
