...
const RETRYABLE_SAVETUBE_STATUS = new Set([408, 429, 500, 502, 503, 504, 520, 522, 524])
const RETRYABLE_SAVETUBE_CODES = new Set(['ECONNABORTED', 'ETIMEDOUT', 'ECONNRESET', 'EAI_AGAIN'])
...
async function savetube(link, quality, value) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            const cdn = (await axios.get("https://media.savetube.vip/api/random-cdn", { timeout: 12000 })).data?.cdn
            if (!cdn) {
                lastError = new Error('CDN no disponible')
                continue
            }
            ...
            if (!infoget?.data) {
                lastError = new Error('No se obtuvo metadata cifrada')
                continue
            }

            const info = decode(infoget.data)
            if (!info?.key) {
                lastError = new Error('No se obtuvo clave de descarga')
                continue
            }
            ...
            if (!downloadUrl) {
                lastError = new Error('No se obtuvo URL de descarga')
                continue
            }
            ...
            const retriable = RETRYABLE_SAVETUBE_STATUS.has(status) || RETRYABLE_SAVETUBE_CODES.has(code)

            lastError = new Error(status ? `Request failed with status code ${status}` : error.message)
            console.warn(`SaveTube intento ${attempt}/${maxAttempts} falló: ${lastError.message}`)
    ...
    return {
        status: false,
        message: lastError?.message || 'Error temporal en SaveTube'
    }
}
