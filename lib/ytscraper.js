...
async function savetube(link, quality, value) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            const cdn = (await axios.get("https://media.savetube.vip/api/random-cdn", { timeout: 10000 })).data?.cdn
            if (!cdn) {
            ...
            const retriable = RETRYABLE_SAVETUBE_STATUS.has(status) || code === 'ECONNABORTED'
