...
async function loadCommandsAndPlugins(dir = commandsFolder) {
    ...
    for (const fileOrFolder of items) {
        ...
        try {
            ...
            // --- CORRECCIÓN 1: Middleware (before/after) ---
            // Eliminamos la restricción "!imported.default?.command"
            // Ahora carga 'before' SIEMPRE que exista la función.
            if (typeof imported.before === 'function' || typeof comando?.before === 'function') {
                global.middlewares.before.push(imported.before || comando.before)
            }

            if (typeof imported.after === 'function' || typeof comando?.after === 'function') {
                global.middlewares.after.push(imported.after || comando.after)
            }

            // --- CORRECCIÓN 2: Validación de Array ---
            if (!comando?.command || typeof comando.run !== "function") continue

            // Solo ejecutamos forEach si ES UN ARRAY. Si es Regex, lo salta (para no crashear).
