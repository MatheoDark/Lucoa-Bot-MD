...
// 🔧 FIX: Almacenar sockets activos para limpiar listeners al reconectar
...
export async function startSubBot(
  ...
  // 🔧 FIX: Limpiar socket anterior para evitar listeners duplicados
  ...
  if (oldSock) {
    ...
    // Esperar a que el socket viejo se libere
...
const sock = makeWASocket({
  ...
  generateHighQualityLinkPreview: false,   // 🔧 FIX v6: false para reducir carga
  ...
  version,
  keepAliveIntervalMs: 30000,    // 🔧 FIX v6: 30s = default (era 60s, causaba 408)
  connectTimeoutMs: 60000,       // 🔧 FIX v6: 60s timeout de conexión
  retryRequestDelayMs: 500,      // 🔧 FIX v6: default Baileys
  maxMsgRetryCount: 3,
...
delete commandFlags[senderId]
    ...
    if (connection === 'close') {
      ...
      if (
        ...
        setTimeout(() => {
          startSubBot(m, client, caption, isCode, phone, chatId, {}, isCommand)
        }, 15000) // 🔧 FIX: 15s en vez de 3s para evitar loops de reconexión rápida
        return
      }

      setTimeout(() => {
        startSubBot(m, client, caption, isCode, phone, chatId, {}, isCommand)
      }, 15000) // 🔧 FIX: 15s en vez de 3s
    }
