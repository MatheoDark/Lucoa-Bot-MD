...
// Referencia al client activo
export let _activeClient = null
...
> ¡Escribe *#drop* o *#recoger* para tomarlo!`
  ...
  // Auto-expirar después de DROP_EXPIRY_MINUTES minutos
  ...
  setTimeout(() => {
    if (activeDrops.has(chatId)) {
      ...
      if (current.timestamp === dropTimestamp) {
        ...
        const currentClient = _activeClient || client
        currentClient.sendMessage(chatId, {
...
export function startDropScheduler(client) {
  ...
  // No limpiamos activeDrops, ya están en pantalla
  ...
  const ejecutarCheck = () => {
    try {
      ...
      // Eliminamos el límite artificial de drops por check para que todos los grupos
      // reciban su drop en la franja correspondiente al mismo tiempo.
      let dropsEnviados = 0
      const hora = moment().tz(TIMEZONE).hour()
