...
// Ciclo de 2 días — GLOBAL para todos los grupos
const CYCLE_DAYS = 2
 ...
 * Drops caen cada 2 días contando desde el epoch.
 */
...
export function startDropScheduler(client) {
  ...
  const ejecutarCheck = () => {
    try {
      ...
      // 🔧 FIX v7: Limitar drops por ciclo para no saturar el stream de WA
      const MAX_DROPS_PER_CHECK = 3
      let dropsEnviados = 0
      const hora = moment().tz(TIMEZONE).hour()
        ...
        for (const franja of FRANJAS) {
          ...
          const tid = setTimeout(async () => {
            try {
              ...
              // 🔧 FIX v8: Solo marcar franja como enviada si el mensaje llegó a WhatsApp
              if (enviado && global.db?.data?.chats?.[groupId]) {
      ...
      // 📊 Resumen detallado cuando no hay drops
      if (dropsEnviados === 0) {
        const franjaStatus = FRANJAS.map(f => {
          ...
          if (enviada) return `${f.emoji}${f.label}: ✅ enviado`
          if (disponible) return `${f.emoji}${f.label}: ⚠️ disponible pero no enviado`
          return `${f.emoji}${f.label}: ⏳ ${f.horaInicio}:00h`
        }).join(' | ')
        console.log(`🎁 [Drops] ${horaLocal} — Check: ${gruposActivos.length} grupos, 0 drops | ${franjaStatus}`)
      } else {
