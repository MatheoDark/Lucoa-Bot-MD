...
// Intervalo de drops: 3 días en milisegundos
const DROP_INTERVAL = 3 * 24 * 60 * 60 * 1000

/**
 * Inicia el scheduler de drops aleatorios
 * Los drops ocurren cada 3 días por grupo.
 * Se puede desactivar por grupo con la opción "drops".
 */
export function startDropScheduler(client) {
  console.log('🎁 Scheduler de drops aleatorios iniciado (cada 3 días por grupo).')

  // Verificar cada 30 minutos si algún grupo necesita un drop
  setInterval(() => {
    ...
    const gruposActivos = Object.entries(chats)
      .filter(([id, data]) => {
        // Solo grupos con RPG activado Y drops activados
    ...
    for (const [groupId, data] of gruposActivos) {
      ...
      const lastDrop = data.lastDrop || 0
      const elapsed = now - lastDrop

      // Si pasaron 3 días (o nunca hubo un drop), lanzar uno
      if (elapsed >= DROP_INTERVAL) {
        const delay = randomInt(0, 5 * 60 * 1000) // 0-5 min de delay aleatorio
        setTimeout(() => {
          spawnDrop(client, groupId)
          // Guardar timestamp del último drop en el grupo
          if (global.db?.data?.chats?.[groupId]) {
            global.db.data.chats[groupId].lastDrop = Date.now()
            if (typeof global.markDBDirty === 'function') global.markDBDirty()
          }
        }, delay)
      }
    }
  }, 30 * 60 * 1000) // Verifica cada 30 minutos
  ...
  setTimeout(() => {
    ...
    // Buscar el grupo que lleva más tiempo sin drop
    let mejorGrupo = null
    let mayorEspera = 0
    for (const [id, data] of gruposActivos) {
      const lastDrop = data.lastDrop || 0
      const espera = now - lastDrop
      if (espera >= DROP_INTERVAL && espera > mayorEspera) {
        mejorGrupo = id
        mayorEspera = espera
      }
    }
    ...
    if (mejorGrupo) {
      spawnDrop(client, mejorGrupo)
      if (global.db?.data?.chats?.[mejorGrupo]) {
        global.db.data.chats[mejorGrupo].lastDrop = Date.now()
        if (typeof global.markDBDirty === 'function') global.markDBDirty()
      }
    }
  }, 2 * 60 * 1000)
