...
const canalSeleccionado = canales[Math.floor(Math.random() * canales.length)]
    ...
    const dynamicButtons = buttons.map((btn) => ({
      ...
      contextInfo: {
        ...
        externalAdReply: {
          title: botname,
          body: botname2 + ', Built With 🐉 By Lucoa',
          mediaType: 1,
  ...
  const pfp = botSetting.icon || ''
    ...
    const canalNam = botSetting.nameid || '──̄͟͞⛛̵̅𝐌͡LUCOA̤𝐁̷𝐨̶̇͜𝐭𓊓̴̻𝐂̷𝐡̶͡𝐚𝐧̈͜𝐧͜𝐞͜𝐥̵̲͞🔥̵̄͟'
      ...
      jid,
      {
        extendedTextMessage: {
          ...
          contextInfo: {
            ...
            externalAdReply: {
              title: namebugsito,
              body: `${namebugg2}, Built With 🔥 By MatheoDark`,
              mediaType: 1,
              renderLargerThumbnail: false,
              previewType: `PHOTO`,
              thumbnailUrl: pfp,
  ...
  client.sendContextInfoIndex = async (
    ...
    const namebot2 = config.body || `${settings.namebot2}, Built With 🔥 By MatheoDark`
    const canalId = settings.id ||'120363358338732714@newsletter'
    const canalName = settings.nameid || '──̄͟͞⛛̵̅𝐌LUCOA̤𝐁̷𝐨̶̇͜𝐭𓊓̴̻𝐂̷𝐡̶͡𝐚𝐧̈͜𝐧͜𝐞͜𝐥̵̲͞🔥̵̄͟'
    const sourceUrl =
