...
import GraphemeSplitter from 'grapheme-splitter' // ✅ AÑADIDO: Vital para emojis
...
export async function smsg(client, m, store) {
    ...
    if (m.message) {
        ...
        // --- ✅ MEJORA DE TU AMIGO: PREFIJOS CON EMOJIS (GraphemeSplitter) ---
        ...
        m.timestamp = (typeof m.messageTimestamp === 'number' ? m.messageTimestamp : m.messageTimestamp?.low ?? m.messageTimestamp?.high ?? 0) || m.msg?.timestampMs * 1000 || Math.floor(Date.now() / 1000)
        ...
        if (m.quoted) {
            ...
            // Prefijo en quoted (También mejorado)
            let quotedPrefix = ''
            if (activePrefixes.length > 0) {
                for (const p of activePrefixes) {
    ...
    m.reply = async (content, options = {}) => {
        ...
        if (typeof content === 'object') {
            ...
            try {
                if (/^https?:\/\//.test(content) && !options.text) { 
    ...
    client.getName = (jid, withoutContact = false) => {
        ...
        else v = id === '0@s.whatsapp.net' ? { id, name: 'WhatsApp' } : id === client.decodeJid(client.user.jid) ? client.user : store.contacts[id] || {}
        return ((withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international'))
    }
    
    // --- TU CÓDIGO ORIGINAL (getFile) ---
    // Lo mantengo porque tu bot puede depender de esto para funciones internas
    ...
    // --- ✅ AÑADIDO: SENDFILE MEJORADO (DE TU AMIGO) ---
    // Este es mucho mejor para enviar multimedia sin preocuparse del tipo
    ...
    client.sendButton = async (jid, text = '', footer = '', buffer, buttons, copy, urls, quoted, options,) => {
        ...
        if (buffer && /^https?:\/\//i.test(buffer)) {
            ...
            } catch (error) { console.error('Error MIME:', error) }
        } else if (buffer) {
             try {
        ...
        if (canales.length > 0) {
            canalSeleccionado = canales[Math.floor(Math.random() * canales.length)]
        } else {
            const settingsId = botSettings.id || '120363423354513567@newsletter'
            const settingsName = botSettings.nameid || '✨ Lucoa Updates ✨'
            canalSeleccionado = { id: settingsId, nombre: settingsName }
        }
        ...
        const interactiveMessage = proto.Message.InteractiveMessage.create({
            ...
            contextInfo: {
                ...
                externalAdReply: {
                    title: botname,
                    body: botname2 + ', Built With 🐉 By MatheoDark',
                    mediaType: 1,
                    renderLargerThumbnail: false,
                    previewType: 'PHOTO',
                    thumbnailUrl: icon,
                    sourceUrl: botSettings.link || 'https://github.com/MatheoDark',
                },
            },
        })
        ...
        console.log('[sendButton] Sending to:', jid, 'messageId:', msgL.key.id)
        console.log('[sendButton] buttons:', JSON.stringify(dynamicButtons))
        try {
            const result = await client.relayMessage(jid, msgL.message, { messageId: msgL.key.id, ...options })
            console.log('[sendButton] relayMessage result:', result)
        } catch (err) {
            console.error('[sendButton] relayMessage ERROR:', err)
        }
    }
    ...
    // --- TU LÓGICA DE NEWSLETTER ORIGINAL ---
    client.newsletterMsg = async (key, content = {}, timeout = 5000) => {
        ...
        if (react) {
            ...
            const hasil = await client.query({
                tag: 'message',
                attrs: { to: key, type: 'reaction', server_id: id, id: generateMessageID(), },
            ...
            const anu = await client.query({
                ...
                content: [{ tag: 'plaintext', attrs: /image|video|audio|sticker|poll/.test(Object.keys(media).join('|')) ? { mediatype: Object.keys(media).find((key) => ['image', 'video', 'audio', 'sticker', 'poll'].includes(key), ) || null, } : {}, content: proto.Message.encode(msg).finish(), }, ],
            })
            return anu
        } else {
        ...
        let prep = generateWAMessageFromContent(jid, {
            extendedTextMessage: {
                ...
                contextInfo: {
                    ...
                    isForwarded: true,
                    forwardedNewsletterMessageInfo: {
                        newsletterJid: botSettings.id || '120363358338732714@newsletter',
                        serverMessageId: '0',
                        newsletterName: botSettings.nameid || 'Lucoa Channel',
                    },
                    externalAdReply: {
                        title: botname,
                        body: botname2 + ', Built With 🐉 By MatheoDark',
    ...
    client.sendContextInfoIndex = async (jid, text = '', options = {}, quoted = null, useQuoted = true, mentionedJid = null, config = {}) => {
        const banner = config.banner || icon
        const botnam = config.title || botname
        const namebot2Text = config.body || (botname2 + ', Built With 🐉 By MatheoDark')
        const canalId = botSettings.id || '120363358338732714@newsletter'
        const canalName = botSettings.nameid || 'Lucoa Channel'
        const sourceUrl = typeof config.redes === 'string' ? config.redes : (botSettings.link || 'https://github.com/MatheoDark')
