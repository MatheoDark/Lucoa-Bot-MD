...
export async function smsg(client, m, store) {
    ...
    // ✅ MEJORADO: Función react más robusta para móviles
    m.react = (emoji) => {
        try {
            // Validar que el key sea válido
            if (!m.key || !m.key.id) {
                console.warn('⚠️ Key inválido para react')
                return Promise.resolve()
            }
            // Enviar reacción sin esperar (fire and forget)
            ;(async () => {
                try {
                    ...
                    console.warn(`⚠️ Error enviando reaction:`, err.message)
                }
            })()
            return Promise.resolve()
        } catch (err) {
            console.warn(`⚠️ Error en m.react:`, err.message)
            return Promise.resolve()
        }
    }
    ...
    client.sendButton = async (jid, text = '', footer = '', buffer, buttons, copy, urls, quoted, options,) => {
        ...
        // --- TU LÓGICA ORIGINAL DE CANALES (global.my) ---
        ...
        const canalSeleccionado = canales.length > 0 ? canales[Math.floor(Math.random() * canales.length)] : { id: '0@newsletter', nombre: 'Lucoa Channel' }
