...
async function startBot() {
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.badSession || reason === DisconnectReason.loggedOut || reason === DisconnectReason.multideviceMismatch) {
        log.warning("Sesión corrupta. Borrando y reiniciando...");
        purgeSession();
    ...
    if (connection === "open") {
        console.log(boxen(chalk.bold(' ¡CONECTADO! '), { borderStyle: 'round', borderColor: 'green', title: chalk.green.bold('● ONLINE ●'), titleAlignment: 'center', float: 'center' }));
    }
  });
  ...
  try { await events(client); } catch (e) {}
